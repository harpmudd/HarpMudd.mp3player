...
## What has actually been played, as opposed to reasoned about

Hardware, 2026-08-15. The first five rows are generated by
`tools/flac_make_test.py`; the rest are real encoder output from the test card.

| case | source | result |
|---|---|---|
| 4096-sample blocks | generated | plays |
| mid/side stereo | generated | plays |
| mono | generated | plays |
| 8-bit | generated | plays |
| 20-bit | generated | plays |
| 1152-sample blocks, 24-bit | generated | plays |
| 16/44.1, 4608 blocks | real | plays |
| 24/44.1, 4608 blocks | real | plays |
| 24/88.2 and 24/96 | real | refused, correctly |

The generated cases matter because the card's real files ALL use 4608-sample
blocks, which is unusual -- the `flac` encoder defaults to 4096, so until this
the block size most real files use had never been decoded once. Mono, 8-bit and
20-bit were claimed in the README on the strength of reading the code.

What this does NOT cover: real encoder output at those settings. The generated
files use VERBATIM subframes, so they exercise the container, the frame header,
the channel modes and the depth conversion -- not Rice or LPC, which the real
files cover thoroughly instead. The two sets are complementary, and neither is
